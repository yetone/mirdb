...
macro_rules! tag {
    ($i:expr, $tag:expr) => {{
        ...
        match compare($i, $tag) {
            CompareResult::Ok => IRResult::Ok(take_split($i, $tag.len())),
            CompareResult::Err => match from_utf8($i) {
                Ok(v) => IRResult::Err(v),
                Err(_) => IRResult::Err("not a valid utf8"),
            },
            CompareResult::Incomplete => IRResult::Incomplete($tag.len() - $i.len()),
        }
    }};
}
...
pub fn digit<T: FromStr>(i: &[u8]) -> IRResult<T> {
    ...
    let (i, o) = match position {
        None => take_split(i, i.len()),
        Some(v) if v > 0 => take_split(i, v),
...
mod test {
    ...
    #[test]
    fn test_is_not() {
        let r = is_not!(b"hello world", b" ").unwrap();
        assert_eq!((" world".as_bytes(), "hello".as_bytes()), r);
    }
    #[test]
    fn test_tag() {
        let r = tag!(b"hello world", b"hello").unwrap();
        assert_eq!((" world".as_bytes(), "hello".as_bytes()), r);
        let r = tag!(b" world", b" ").unwrap();
        assert_eq!(("world".as_bytes(), " ".as_bytes()), r);
    }
    #[test]
    fn test_take() {
        let r = take!(b"hello world", 7).unwrap();
        assert_eq!(("orld".as_bytes(), "hello w".as_bytes()), r);
    }
    #[test]
    fn test_take_incomplete() {
        let r = take!(b"hello world", 12);
        assert_eq!(IRResult::Incomplete(1), r);
    }
    #[test]
    fn test_at_least() {
        let r = take_at_least!(b"hello world", 3, b"").unwrap();
        assert_eq!(("".as_bytes(), "hello world".as_bytes()), r);
        let r = take_at_least!(b"hello world", 3, b"we").unwrap();
        assert_eq!(("".as_bytes(), "hello world".as_bytes()), r);
        let r = take_at_least!(b"hello world", 3, b"wo").unwrap();
        assert_eq!(("world".as_bytes(), "hello ".as_bytes()), r);
        let r = take_at_least!(b"hello world! hello world", 7, b"wo").unwrap();
        assert_eq!(("world".as_bytes(), "hello world! hello ".as_bytes()), r);
    }
    #[test]
    fn test_alpha() {
        let r = alpha(b"hello world").unwrap();
        assert_eq!((" world".as_bytes(), "hello".as_bytes()), r);
        let r = alpha(b" hello world");
    ...
    #[test]
    fn test_digit() {
        let r = digit::<i32>(b"123").unwrap();
        assert_eq!(("".as_bytes(), 123i32), r);
        let r = digit::<usize>(b"123x").unwrap();
        assert_eq!(("x".as_bytes(), 123usize), r);
        let r = digit::<usize>(b"x123");
        assert_eq!(IRResult::Err(""), r);
        let r = digit::<usize>(b"");
        assert_eq!(IRResult::Err(""), r);
    }
    #[test]
    fn test_space() {
        let r = space(b" ").unwrap();
        assert_eq!(("".as_bytes(), " ".as_bytes()), r);
        let r = space(b"   ").unwrap();
        assert_eq!(("".as_bytes(), "   ".as_bytes()), r);
        let r = space(b"   x").unwrap();
    ...
    fn test_chain() {
        let r = chain!(
            b"hello world",
            hello: alpha >> tag!(b" ") >> world: alpha >> (hello, world)
        )
        .unwrap()
        .1;
    ...
    fn test_chain_incomplete() {
        let r = chain!(
            b"hello world",
            hello: alpha >> tag!(b" ") >> world: alpha >> tag!(b" ") >> (hello, world)
        );
        assert_eq!(IRResult::Incomplete(1), r);
    }
    #[test]
    fn test_gen_parser() {
        gen_parser!(
            test<(&[u8], &[u8])>,
            chain!(hello: alpha >> tag!(b" ") >> world: alpha >> (hello, world))
        );
        assert_eq!(
            IRResult::Ok(("".as_bytes(), ("hello".as_bytes(), "world".as_bytes()))),
            test(b"hello world")
        );
        gen_parser!(getter_name<&[u8]>, alt!(tag!(b"gets") | tag!(b"get")));
        assert_eq!(
            IRResult::Ok((" hello world".as_bytes(), "get".as_bytes())),
            getter_name(b"get hello world")
        );
        assert_eq!(
            IRResult::Ok((" hello world".as_bytes(), "gets".as_bytes())),
            getter_name(b"gets hello world")
        );
    }
    #[test]
    fn test_opt() {
        let r = opt!(b"hello world", tag!(b"hello"));
        assert_eq!(
            IRResult::Ok((" world".as_bytes(), Some("hello".as_bytes()))),
            r
        );
        let r = opt!(b"hello world", tag!(b"xixi"));
        assert_eq!(IRResult::Ok(("hello world".as_bytes(), None)), r);
        let r = opt!(b"hello", tag!(b"hello world"));
    ...
    fn test_alt() {
        ...
        let r = alt!(
            b"hello world",
            tag!(b"xixi") | tag!(b"hello") | tag!(b"h") | custom_parser
        );
        ...
        assert_eq!(
            IRResult::Ok((" hello world".as_bytes(), "xixi".as_bytes())),
            r
        );

        let r = alt!(b"get hello world", tag!(b"get") | tag!(b"gets"));
        assert_eq!(
            IRResult::Ok((" hello world".as_bytes(), "get".as_bytes())),
            r
        );

        let r = alt!(b"gets hello world", tag!(b"get") | tag!(b"gets"));
        assert_eq!(
            IRResult::Ok(("s hello world".as_bytes(), "get".as_bytes())),
            r
        );
        let r = alt!(b"gets hello world", tag!(b"gets") | tag!(b"get"));
        assert_eq!(
            IRResult::Ok((" hello world".as_bytes(), "gets".as_bytes())),
            r
        );
    ...
    #[test]
    fn test_split() {
        let r = split!(b"I love u", space, alpha);
        assert_eq!(
            IRResult::Ok((
                "".as_bytes(),
                vec!["I".as_bytes(), "love".as_bytes(), "u".as_bytes()]
            )),
            r
        );
        let r = split!(b"I love u#", space, alpha);
        ...
        let r = split!(b"", space, alpha);
        assert_eq!(IRResult::Err(""), r);
        let r = split!(b" I love u", space, alpha);
        assert_eq!(IRResult::Err(""), r);
        let r = chain!(b" I love u", space >> keys: split!(space, alpha) >> (keys)).unwrap();
        assert_eq!(
            (
                "".as_bytes(),
                vec!["I".as_bytes(), "love".as_bytes(), "u".as_bytes()]
        ...
        let r = chain!(
            b"get I love u",
            getter: getter_name >> space >> keys: split!(space, alpha) >> (getter, keys)
        )
        .unwrap();
        assert_eq!(
            (
