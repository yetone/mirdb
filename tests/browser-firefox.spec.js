...
// Only run these tests in Firefox browser
test.describe('Browser Compatibility - Firefox', () => {
  ...
  test.describe('Test Case 1: Page loads without errors and displays all sections correctly', () => {
    ...
    test('Page returns successful HTTP status in Firefox', async ({ page }) => {
      const response = await page.goto('/');

      // Verify successful HTTP response (200 OK or 304 Not Modified are both valid)
      // Note: response.ok() only returns true for 2xx status codes
      // 304 Not Modified is a valid response indicating the cached version is still current
      const status = response.status();
      expect([200, 304]).toContain(status);
      // For 304, we check that it's a valid "success" response (no error)
      // For 200, response.ok() should be true
      if (status === 200) {
    ...
    test('CSS styles are correctly applied in Firefox', async ({ page }) => {
      ...
      // Firefox may report font-family differently, but should contain system fonts
    ...
    test('CSS Grid layout works correctly in Firefox', async ({ page }) => {
    ...
    test('CSS Flexbox layout works correctly in Firefox', async ({ page }) => {
  ...
  test.describe('Test Case 2: Visual layout matches Chrome rendering', () => {
    test('GIF images load and display correctly in Firefox', async ({ page }) => {
    ...
    test('GIF images use lazy loading attribute in Firefox', async ({ page }) => {
    ...
    test('Value proposition cards have hover effects in Firefox', async ({ page }) => {
    ...
    test('Code blocks display correctly in Firefox', async ({ page }) => {
    ...
    test('Border radius is applied correctly in Firefox', async ({ page }) => {
    ...
    test('Box shadow is applied correctly in Firefox on hover', async ({ page }) => {
      const featureCard = page.locator('.feature-card').first();
    ...
    test('Linear gradient backgrounds render correctly in Firefox', async ({ page }) => {
