 ...
 * and functions properly in Safari browser (using WebKit engine).
 *
 * Test Cases:
 * TC1: Page loads without console errors in Safari
 * TC2: All sections display with correct layout in Safari
 * TC3: Navigation links scroll to sections (smooth or instant)
 */
...
test.describe('Cross-Browser Compatibility - Safari', () => {
  ...
  test('TC1: Page loads without console errors in Safari', async ({ page }) => {
    const consoleErrors: string[] = [];

    // Collect console errors (excluding network errors from external resources like CDN)
    page.on('console', (msg) => {
      if (msg.type() === 'error') {
        const text = msg.text();
        // Filter out network errors from external CDN resources (Prism.js)
        // These are not application errors but infrastructure/network issues
    ...
    // Verify no application JavaScript errors occurred
    expect(consoleErrors.length).toBe(0);
  });

  test('TC2: All sections display with correct layout in Safari', async ({ page }) => {
  ...
  test('TC3: Navigation links scroll to sections (smooth or instant) in Safari', async ({ page }) => {
    ...
    // Wait for scroll animation (Safari may use instant scroll if smooth scroll not supported)
  ...
  test('SVG diagram renders correctly in Safari', async ({ page }) => {
  ...
  test('External links have correct attributes in Safari', async ({ page }) => {
  ...
  test('Fixed navigation stays at top when scrolling in Safari', async ({ page }) => {
  ...
  test('Code blocks render with syntax highlighting in Safari', async ({ page }) => {
  ...
  test('WebKit-specific: CSS transforms render correctly', async ({ page }) => {
    // Feature cards have transform on hover - verify the base state
    const featureCard = page.locator('.feature-card').first();
    await expect(featureCard).toBeVisible();

    // Get the computed transform (should be none or matrix in default state)
    const transform = await featureCard.evaluate((el) => {
      return window.getComputedStyle(el).transform;
    });
    // Default state should have no transform or identity matrix
    expect(transform === 'none' || transform.includes('matrix')).toBe(true);
  });

  test('WebKit-specific: Border radius renders correctly', async ({ page }) => {
    // Verify border radius is applied correctly on cards
    const featureCard = page.locator('.feature-card').first();
  ...
  test('WebKit-specific: Linear gradients render correctly', async ({ page }) => {
    ...
    // Verify the gradient includes color stops
    expect(bgImage).toContain('rgb');
  });

  test('WebKit-specific: Box shadows render correctly', async ({ page }) => {
    // Feature cards have box-shadow
    const featureCard = page.locator('.feature-card').first();
    const boxShadow = await featureCard.evaluate((el) => {
      return window.getComputedStyle(el).boxShadow;
    });
    // Should have a valid box-shadow (not 'none')
