...
test.describe('Mobile Responsive Design (375px viewport)', () => {
  ...
  test('Test Case 4: All sections are accessible and content is visible', async ({ page }) => {
    ...
    // Check that we can scroll (scroll position changed from 0)
    ...
    // We should be able to scroll to near the bottom (allow for some tolerance due to lazy loading, etc.)
    // The key requirement is that all sections are accessible, which we verified above
    expect(scrollPosition.scrollY).toBeGreaterThan(0);
    // Document should be scrollable (height greater than viewport)
    expect(scrollPosition.documentHeight).toBeGreaterThan(scrollPosition.viewportHeight);
  });
