//! Status Tests for Server Status and Version Display
//!
...
fn test_state_version_constant_exists() {
    // The state module exports VERSION constant using env!("CARGO_PKG_VERSION")
    // This verifies the pattern is in state.rs by checking handlers.rs includes it
...
fn test_status_response_has_running_field() {
    let handlers_content = include_str!("../../src/homepage/handlers.rs");
    // Check that the JSON response includes a running/status field
    assert!(
...
fn test_css_status_indicator_is_round() {
    // Check that status indicator has border-radius for circular appearance
    assert!(
...
fn test_css_running_indicator_has_green_color() {
    // The running status should have a green indicator
    assert!(
